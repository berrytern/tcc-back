...
use crate::{application::models::solicitacao::CreateSolicitacaoModel, errors::AppError, infrastructure::database::schemas::solicitacao_schema::{OptionSolicitacaoSchema, SolicitacaoSchema}, port::query_filter::QueryFilter};
...
pub async fn create_solicitacao(app: Data<App>, solicitacao: Json<CreateSolicitacaoModel>) -> Result<impl Responder, AppError> {
    let controller = &app.controllers.solicitacao;
    controller.create_solicitacao(Box::new(solicitacao.into_inner().into())).await
}
// pf:u
#[utoipa::path(tag = "solicitacao", responses((status = OK, body = SolicitacaoSchema)))]
